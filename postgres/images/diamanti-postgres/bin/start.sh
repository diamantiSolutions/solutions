...
if [ ! -v KUBERNETES_SERVICE_PORT ]; then
	echo "KUBERNETES_SERVICE_PORT env var is not set, aborting"
	exit 1
fi

if [ ! -v PG_MASTER_PORT ]; then
	echo "PG_MASTER_PORT env var is not set, aborting"
	exit 1
fi
...
if [ ! -v POD_IP ]; then
	echo "POD_IP env var is not set, aborting"
	exit 1
fi

if [ ! -v POD_NAME ]; then
	echo "POD_NAME env var is not set, aborting"
	exit 1
fi
...
#setup Kubernetes
kubectl config set-cluster solutions-cluster --server=http://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT --api-version=v1
kubectl config set-context solutions --cluster=solutions-cluster
kubectl config use-context solutions
...
#	>>>>>>>>>>>>>>>>>>>>>;
	...
	#call consul API to find master or wait.Is it a blocking call if not will need to loop.
	#consule is running on side-car container on same pod, so can be accessed as localhost.

	response=""
	while [[ $response = "" ]]; do
	    sleep 5
	    response=`curl localhost:8500/v1/status/leader | sed 's/\"//g' `
	    echo "consule master is: $response ."
	done
	
	IFS=':' read -ra master <<< "$response"
	
	export PG_MASTER_HOST=${master[0]};
	
	if [[ ${master[0]} == $POD_IP ]]; then
	    echo "setting PG_MODE to master"
	    export PG_MODE=master
...
function setupRecoveryConf() {

    #delete any old recovery done file.
...
if [ ! -f $PGDATA/postgresql.conf ]; then
    ...
    #if pgconf exists, that means its possible that it was slave/master earlier but now coming back up as master
    #lets promote it to make sure its acting as master.
