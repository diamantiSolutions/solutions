...
sed -i "s/PG_READ_SERVICE_NAME/$PG_READ_SERVICE_NAME/g" $CONFIGS/pgpool.conf
#sed -i "s/PG_SLAVE_SERVICE_NAME1/$PG_SLAVE_SERVICE_NAME1/g" $CONFIGS/pgpool.conf
...
/bin/pgpool -n -a $CONFIGS/pool_hba.conf -f $CONFIGS/pgpool.conf  -d > /tmp/pgpool.log 2>&1 &
export PGPOOL_PID=$!
